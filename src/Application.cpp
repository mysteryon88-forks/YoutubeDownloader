...
LRESULT Application::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IdDownloadButton) {
            SetStatus(L"yt-dlp ещё не подключён: показано окно ошибки.");
            ShowErrorDialog(
                m_window,
                m_instance,
                L"yt-dlp ещё не готов",
                L"Загрузка пока недоступна, потому что слой установки и проверки yt-dlp ещё не подключён.\n\n"
                L"Это тестовое диагностическое сообщение. Оно специально достаточно длинное, чтобы проверить темное окно ошибки, кнопку копирования текста и кастомный scroll-text control. "
                L"В дальнейшем сюда будет попадать реальный текст ошибки из менеджера инструментов, yt-dlp, FFmpeg или сетевого слоя.\n\n"
                L"Код: TOOL_NOT_READY\n"
                L"Компонент: ToolManagers\n"
                L"Действие: download"
            );
            return 0;
        }
        if (LOWORD(wParam) == IdSettingsButton) {
            ShowSettingsDialog(m_window, m_instance);
            return 0;
        }
        break;
